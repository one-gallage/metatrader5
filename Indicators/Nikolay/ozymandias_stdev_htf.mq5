...
#define INDICATOR_NAME "Ozymandias_StDev"             // Константа для имени индикатора
#define SIZE 4                                        // Константа для количества вызовов функции CountLine
//+----------------------------------------------+
...
//--- проверка периодов графиков на корректность
   if(TimeFrame<Period() && TimeFrame!=PERIOD_CURRENT)
     {
      Print("Период графика для индикатора Ozymandias_StDev не может быть меньше периода текущего графика");
      return(INIT_FAILED);
     }
...
//--- получение хендла индикатора Ozymandias_StDev
   Ind_Handle=iCustom(Symbol_,TimeFrame,"Ozymandias_StDev",Length,MAType,dK,std_period,0);
   if(Ind_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора Ozymandias_StDev");
      return(INIT_FAILED);
     }
